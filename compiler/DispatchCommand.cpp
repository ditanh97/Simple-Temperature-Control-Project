...
dispatchCommand::dispatchCommand(bool* Q1state, bool* Q2state, int pinT1){
_Q1state = Q1state;
_Q2state = Q2state;
_pinT1 = pinT1;
//default
...
float dispatchCommand::getTemp(){
for (int i = 0; i < _n; i++) {
_mV = (float) analogRead(_pinT1) * (3300.0/1024.0);
    _degC = _degC + (_mV - 500.0)/10.0;
    }
  _degC = _degC / float(_n);
  return _degC;
}
